...
def convertir_fecha_segura(fecha_str):
    try:
        return pd.to_datetime(fecha_str, format='%Y-%m-%d').date()
...
class Empresa(DataModel):
    ...
    def existe(self, rut):
        """Verifica si una empresa existe por su RUT (normalizado)"""
        if self.df.empty:
            return False
        # Asegurar que los RUTs en el DataFrame están normalizados
        self.df['rut'] = self.df['rut'].apply(normalizar_rut)
        return rut in self.df['rut'].values  # <-- Comparar con RUTs normalizados

    def crear_nueva(self, rut, datos):
        """Crea nueva empresa con RUT normalizado"""
        if not self.validate_rut(rut):
            raise ValueError("RUT inválido")
        rut_normalizado = normalizar_rut(rut)
        if self.check_duplicates(self.df, 'rut', rut_normalizado):
            raise ValueError("Empresa ya existe")
        new_row = {
            'rut': rut_normalizado,  # Almacenar RUT normalizado
...
def validar_formulario_accidente(fecha, hora):
    hoy = datetime.date.today()
    if fecha > hoy:
        st.error("La fecha del accidente no puede ser futura")
        return False
    if hora > datetime.datetime.now().time():
        st.error("La hora del accidente no puede ser futura")
        return False
    return True

def confirmar_salida():
    if st.session_state.get("unsaved_changes"):
        return st.confirm("⚠️ Tienes cambios sin guardar. ¿Seguro que quieres salir?")
    return True


def verificar_estado(pagina_actual):
    requerimientos = {
        "seleccionar_centro": ["rut_empresa"],
        "nuevo_centro": ["rut_empresa"],
        "accidentes": ["cuv"],
        "trabajadores": ["accidente_id"],
        "investigacion": ["accidente_id"],
        "arbol_causas": ["accidente_id"],
        "documentos": ["accidente_id"]
    }

    if pagina_actual in requerimientos:
        for campo in requerimientos[pagina_actual]:
            if campo not in st.session_state or not st.session_state[campo]:
                st.error(f"Falta información requerida: {campo}")
                st.session_state.pagina = "inicio"
                st.stop()  # Se detiene la ejecución, permitiendo recargar la página con el estado corregido
...
def pagina_inicio():
    ...
    # Solo si se ha realizado la búsqueda correctamente, mostrar centros de trabajo y navegación
    if st.session_state.busqueda_realizada:
        empresa = Empresa()  # Se puede volver a instanciar o conservar la instancia
        ...
        if not centros.empty:
            ...
            options = ["Seleccione..."] + list(centros['nombre_centro'])
            ...
            with st.expander("Vista previa del centro seleccionado"):
                if selected_centro == "Seleccione...":
                    st.info("Por favor, seleccione un centro para ver su información.")
                else:
            ...
            # Opciones para continuar o crear un nuevo centro
            col1, col2 = st.columns([3, 2])
            with col1:
                if st.button("Usar Centro Seleccionado", key="btn_usar_centro"):
                    if selected_centro != "Seleccione...":
                        ...
                        st.error("Debe seleccionar un centro válido.")
            with col2:
                if st.button("Crear Nuevo Centro", key="btn_crear_centro"):
...
# Función de validación de RUT corregida
def validar_rut(rut):
    pattern = r'^0*(\d{1,3}(\.?\d{3}){2}-[\dkK])$'
    return re.match(pattern, rut, re.IGNORECASE) is not None

def pagina_seleccionar_centro():
    st.header("Selección de Centro de Trabajo")
...
def mostrar_ruta_navegacion():
    # Diccionario de relaciones entre páginas y sus rutas
    breadcrumbs = {
        "inicio": [],
        "nueva_empresa": ["inicio"],
        "nuevo_centro": ["inicio"],
        "accidentes": ["inicio"],
        "nuevo_accidente": ["inicio", "accidentes"],
        "editar_accidente": ["inicio", "accidentes"],
        "trabajadores": ["inicio", "accidentes"],
        "investigacion": ["inicio", "accidentes"],
        "arbol_causas": ["inicio", "accidentes", "investigacion"],
        "documentos": ["inicio", "accidentes", "arbol_causas"]
    }
    ...
    nombres_paginas = {
        ...
        "nuevo_centro": "Nuevo Centro de Trabajo",
        "accidentes": "Gestión de Accidentes",
        "nuevo_accidente": "Nuevo Accidente",
        "editar_accidente": "Editar Accidente",
        "trabajadores": "Trabajadores Involucrados",
        "investigacion": "Investigación del Accidente",
    ...
    ruta = breadcrumbs.get(pagina_actual, [])
    ruta_completa = ruta + [pagina_actual]

    # Crear columnas dinámicas
    num_elementos = len(ruta_completa)  # <-- Variable correcta
    columnas = st.columns([1] + [0.2] * (num_elementos - 1))  # <-- Usar nombre correcto

    with columnas[0]:
        st.write("**Ruta de navegación:**")

    for i, pagina in enumerate(ruta_completa):
        nombre_mostrar = nombres_paginas.get(pagina, pagina)
        with columnas[i + 1 if i > 0 else 0]:
            if i == len(ruta_completa) - 1:
                # Página actual (no clickeable)
                st.markdown(f"**› {nombre_mostrar}**")
            else:
                if st.button(f"› {nombre_mostrar}", key=f"breadcrumb_{pagina}"):
                    st.session_state.pagina = pagina
                    st.stop()  # Se detiene para recargar la página con la nueva ruta


def normalizar_rut(rut):
    # Eliminar todos los caracteres no alfanuméricos excepto 'K' y convertir a mayúsculas
    rut_clean = re.sub(r'[^0-9kK]', '', rut).upper()
    if len(rut_clean) < 2:
        return rut  # Si es inválido, se manejará en la validación posterior
    cuerpo = rut_clean[:-1]
    dv = rut_clean[-1]
    # Invertir el cuerpo para agregar puntos cada 3 dígitos desde la derecha
    cuerpo_invertido = cuerpo[::-1]
    partes = []
    for i in range(0, len(cuerpo_invertido), 3):
        partes.append(cuerpo_invertido[i:i+3])
    cuerpo_con_puntos = '.'.join(partes)[::-1]
...
def main():
    st.set_page_config(
        layout="wide",
        page_title="Sistema de Gestión de Accidentes Laborales",
        page_icon="⚠️"
    )
    ...
    paginas = {
        ...
        "seleccionar_centro": pagina_seleccionar_centro,
    ...
    verificar_estado(st.session_state.pagina)

    # 3. Componentes de UI
    #if st.session_state.get("rut_empresa"):
    #    mostrar_menu_lateral()

    if st.session_state.pagina != "inicio":
        mostrar_ruta_navegacion()
        st.divider()

    # 4. Cargar página actual
    try:
    ...
    # 5. Footer
    st.divider()
    st.caption("Sistema de Gestión de Accidentes Laborales v1.0 | © 2025")
